...
(ns ^:no-doc from.potemkin.collections
  (:use
    [from.potemkin types macros utils]))
...
(def-abstract-type AbstractMap
  ...
  from.potemkin.collections.PotemkinMap
  ...
  clojure.lang.IObj
  (withMeta [this mta]
    (from.potemkin.collections/with-meta* this mta))
  (meta [this]
    (from.potemkin.collections/meta* this))
  (meta* [this]
    nil)
  ...
  (count [this]
    (count (from.potemkin.collections/keys* this)))

  clojure.lang.Seqable
  (seq [this]
    (seq
      (map
        #(from.potemkin.PersistentMapProxy$MapEntry. this %)
        (from.potemkin.collections/keys* this))))
  ...
  (hasheq [this]
    (from.potemkin.collections/compile-if (resolve 'clojure.core/hash-unordered-coll)
  ...
  (valAt [this k default]
    (from.potemkin.collections/get* this k default))
  ...
  (entryAt [this k]
    (when (contains? (.keySet this) k)
      (from.potemkin.PersistentMapProxy$MapEntry. this k)))

  (assoc [this k v]
    (from.potemkin.collections/assoc* this k v))

  (empty* [this]
    {})

  (empty [this]
    (from.potemkin.collections/empty* this))
  ...
  (size [this]
    (count this))
  (keySet [this]
    (set (from.potemkin.collections/keys* this)))
  (put [_ _ _]
  ...
  (assocEx [this k v]
    (if (contains? this k)
      ...
      (assoc this k v)))
  (without [this k]
    (from.potemkin.collections/dissoc* this k))

  from.potemkin.collections/PotemkinFn

  (invoke [this k]
    (from.potemkin.collections/get* this k nil))
...
(defmacro def-map-type
  ...
  (let [fns '{get get*
    ...
    (unify-gensyms
      `(do
         (deftype+ ~name ~params ~'from.potemkin.collections/AbstractMap
           ~@(map
               #(if (sequential? %)
...
(defmacro reify-map-type
  ...
  (let [fns '{get get*
        ...
        elide? '#{withMeta meta}]
    (->>
      `(reify+ ~'from.potemkin.collections/AbstractMap
         ~@(map
             #(if (sequential? %)
                (list* (get fns (first %) (first %)) (rest %))
                %)
