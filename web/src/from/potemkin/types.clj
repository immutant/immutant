...
(defn abstract-type? [x]
  (and (symbol? x) (= :from.potemkin/abstract-type (-> x safe-resolve meta :tag))))
...
(defmacro def-abstract-type
  ...
  [name & body]
  `(def
     ~(with-meta name {:tag :from.potemkin/abstract-type})
     '(deftype ~name [] ~@body)))

(defmacro defprotocol+
  "A protocol that won't evaluate if an equivalent protocol with the same name already exists."
  [name & body]
  (let [prev-body (-> name resolve meta :from.potemkin/body)]
    (when-not (equivalent? prev-body body)
      `(let [p# (defprotocol ~name ~@body)]
         (alter-meta! (resolve p#) assoc :from.potemkin/body '~(r/macroexpand-all body))
         p#))))
...
(defmacro deftype+
  ...
  [name params & body]
  (let [body (->> (list* 'deftype name params 'from.potemkin.types.PotemkinType body)
               clean-deftype
               expand-deftype
               deftype*->deftype)
...
(defmacro reify+
  ...
  [& body]
  (let [body (->> (list* 'deftype (gensym "reify") [] 'from.potemkin.types.PotemkinType body)
               clean-deftype
               expand-deftype
               deftype*->deftype)]
