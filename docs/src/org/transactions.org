...
** Transaction Scope
   ...
   So instead, [[./apidoc/immutant.xa.html#var-/docs.oracle.com/javaee/6/tutorial/doc/bncij.html][the JEE transaction attributes]] in the form of Clojure
   macros. In fact, the =xa/transaction= macro shown above is merely
   an alias for [[./apidoc/immutant.xa.transaction.html#var-required][immutant.xa.transaction/required]], which is the
