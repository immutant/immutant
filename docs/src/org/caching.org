...
** Creating
   ...
   running in the same Immutant or cluster. The
   =immutant.cache/lookup= function is provided to obtain a reference
   to an existing cache without having to recreate it and clear all
   its data. If the cache doesn't exist, nil is returned. There is
   also a convenience function, =immutant.cache/lookup-or-create= that
   will only create the cache if it doesn't already exist. It accepts
   the same options as =create=.
