...
* Application Deployment
  ...
  middleware, are enabled when the =:dev= profile is active, which is
  the default. We recommend deploying your applications to production
  with a non-dev profile activated, e.g.

  #+begin_src sh
      $ lein with-profile production immutant deploy
      # or
      $ lein with-profile production immutant archive
  #+end_src

  Alternatively, you could manually add a =:lein-profiles= entry to
  your deployment descriptor. This is the file that gets created by
  the =deploy= task. See the [[./initialization.html][initialization]] and [[./deployment.html][deployment]] chapters
  for more details.
