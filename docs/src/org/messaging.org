...
** Accessing Destinations via JMX
   ...
   Messaging destinations have MBeans exposed via JMX. From there, you
   can see message counts, list & remove messages, and perform other
   operations. Two different MBeans are provided for each destination,
   each with slightly different operations. One is a JMS MBean, and
   the other is HornetQ specific.
