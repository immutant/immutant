...
** Publishing
   ...
   |---------------+----------------------------------+------------|
   |               | <32>                             | <c>        |
   | =:encoding=   | Either =:clojure=, =:json= or =:text= | =:clojure= |
   | =:priority=   | An integer (0-9) or one of =:low=, =:normal=, =:high= and =:critical= which correspond to 0, 4, 7 and 9, respectively | 4          |
   | =:ttl=        | An integer greater than 0, indicating the number of milliseconds after which the message is discarded if not consumed | 0          |
...
** Receiving
   ...
   |-------------+----------------------------------+---------|
   |             | <32>                             | <c>     |
   | =:timeout=  | An expiration in milliseconds, after which nil is returned; a value of 0 means wait forever | 10000   |
   ...
   |----------------+----------------------------------+---------|
   |                | <32>                             | <c>     |
   | =:concurrency= | The maximum number of listening threads that can simultaneouly call the function | 1       |
