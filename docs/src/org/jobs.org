...
* Scheduling Jobs
  ...
  + =options= - options are specified as alternating key & value
    ...
    | :in        | /none/                              | Specifies when the /at/ job should start firing, in ms from now, a period alias, or a period spec (see below). Can't be specified with a =spec= or =:at=.                                                                   |
    | :every     | /none/                              | Specifies the delay interval between /at/ job firings, in ms, a period alias, or a period spec (see below). If specified without a =:repeat= or =:until=, the job will fire indefinitely. Can't be specified with a =spec=.  |
  ...
  =:day=, =:week=. A period spec is a vector or list with a number
