...
* The Initialization Process
  ...
  6. At this point, Immutant creates a Clojure runtime 
...
** A note on profiles
   ...
   strongly urge you to [[#initialization-configuration][explicitly specify]] the profiles you want
   active, which will override the default set. Neither =:base= nor
   =:user= are appropriate profiles for production, since =:base= will
   bring in [[https://github.com/technomancy/leiningen/blob/stable/doc/TUTORIAL.md#checkout-dependencies][checkout dependencies]], and =:user= will [[https://github.com/technomancy/leiningen/blob/stable/doc/PROFILES.md#declaring-profiles][apply user level
   settings]] from =~/.lein/profiles.clj=.
