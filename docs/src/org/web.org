...
* Locating dirs within the application root
  ...
  application root. You can do so via the [[./apidoc/immutant.utilities.html#immutant.utilities/app-root][immutant.utilities/app-root]] 
  function.
...
** An example
  ...
  #+begin_src clojure
    (use 'ring.middleware.reload-modified)
    (require '[immutant.utilites :as util]
             '[clojure.java.io :as io])
    
    (def app
      (wrap-reload-modified #'handler [(io/file (app/root) "src")]))
  #+end_src
