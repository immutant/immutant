...
* src-dir
  ...
  at runtime, and that's why [[./apidoc/immutant.web-api.html#immutant.web/src-dir][immutant.web/src-dir]] exists: it returns
  the absolute path to a directory relative to the location of the
  file in which it's called. That sounds complicated enough for an
  example, right?
